    ...
    if model is not None:
        try:
            ...
            with st.spinner('🔄 Analyzing patient data...'):
                ...
                # CRITICAL FIX: Ensure we're using the right probability for the positive class
                # If prediction_label is 1 (angina), prediction_score should be the probability of angina
                # If prediction_label is 0 (no angina), prediction_score should be 1 - probability of angina
                ...
                with col2:
                    if prediction_label == 1:
                        ...
                        <div class="prediction-card high-risk">
                            ...
                            <p style="font-size: 1.2rem;">Risk Probability: {angina_probability:.1%}</p>
                ...
                # Enhanced results with confidence and probability breakdown
                st.subheader("📊 Detailed Analysis")
                
                col_prob1, col_prob2 = st.columns(2)
                with col_prob1:
                    st.metric("Angina Risk Probability", f"{angina_probability:.1%}")
                with col_prob2:
                    confidence = max(angina_probability, 1 - angina_probability)
                    st.metric("Model Confidence", f"{confidence:.1%}")
                
                # Probability breakdown table
                results_df = pd.DataFrame({
                    'Outcome': ['No Anginal Pain', 'Anginal Pain'],
                    'Probability': [f"{1-angina_probability:.1%}", f"{angina_probability:.1%}"]
                })
                st.dataframe(results_df, use_container_width=True)
                
                # Enhanced risk interpretation
                st.subheader("🎯 Risk Interpretation")
                if angina_probability < 0.3:
                    st.success("🟢 **Low Risk**: The patient has a low probability of experiencing anginal chest pain. Continue routine monitoring.")
                elif angina_probability < 0.7:
                    st.warning("🟡 **Moderate Risk**: The patient has a moderate probability of experiencing anginal chest pain. Consider further evaluation and monitoring.")
                else:
                    st.error("🔴 **High Risk**: The patient has a high probability of experiencing anginal chest pain. Immediate medical attention and comprehensive cardiac evaluation recommended.")
                
                # Risk gauge
                st.subheader("📊 Risk Assessment Gauge")
                gauge_chart = create_gauge_chart(angina_probability)
                ...
                with st.expander("📋 Patient Data Summary"):
                    st.write("**Complete Patient Information Used for Prediction:**")
                    
                    # Organize input summary by categories
                    ...
                    with col_sum1:
                        ...
                        st.write(f"- Ethnicity: {inputs['ethnic']}")
                        st.write(f"- BMI: {inputs['BMI']:.1f} kg/m²")
                        st.write(f"- Smoking: {inputs['smoking_status']}")
                        st.write(f"- Physical Activity: {inputs['physical_activity']}")
                        st.write(f"- Chest Pain: {'Yes' if inputs['chest_pain'] else 'No'}")
                        
                        st.write("**Vital Signs:**")
                        st.write(f"- Blood Pressure: {inputs['mean_sbp']}/{inputs['mean_dbp']} mmHg")
                        st.write(f"- Heart Rate: {inputs['mean_heart_rate']} bpm")
                        
                        st.write("**Medical History:**")
                        st.write(f"- Family CHD: {'Yes' if inputs['fam_chd'] else 'No'}")
                        st.write(f"- Diabetes: {inputs['diabetes_status']}")
                        st.write(f"- Hypertension Treatment: {'Yes' if inputs['treated_hypertension'] else 'No'}")
                        st.write(f"- Cholesterol Medication: {'Yes' if inputs['chol_lowering'] else 'No'}")
                    
                    with col_sum2:
                        st.write("**Key Laboratory Values:**")
                        st.write(f"- HbA1c: {inputs['hba1c']}")
                        st.write(f"- Glucose: {inputs['glucose']:.1f} mmol/L")
                        st.write(f"- Total Cholesterol: {inputs['total_cholesterol']:.1f} mmol/L")
                        st.write(f"- HDL: {inputs['hdl']:.1f} mmol/L")
                        st.write(f"- LDL: {inputs['ldl']:.1f} mmol/L")
                        st.write(f"- Triglycerides: {inputs['triglyceride']:.1f} mmol/L")
                        st.write(f"- Cholesterol/HDL Ratio: {inputs['Cholesterol_HDL_Ratio']:.1f}")
                        
                        st.write("**Kidney & Liver Function:**")
                        st.write(f"- Creatinine: {inputs['creatinine']} μmol/L")
                        st.write(f"- BUN: {inputs['blood_urea_nitrogen']:.1f} mmol/L")
                        st.write(f"- AST: {inputs['ast']:.1f} U/L")
                        
                        st.write("**Blood Count:**")
                        st.write(f"- Hemoglobin: {inputs['hemoglobin']:.1f} g/dL")
                        st.write(f"- Hematocrit: {inputs['hematocrit']:.1f}%")
                        st.write(f"- WBC: {inputs['white_blood_cell_count']:.1f} ×10³/μL")
                        st.write(f"- Platelets: {inputs['platelet_count']:.0f} ×10³/μL")
            ...
            st.error("Please check that all input values are valid and the model is properly loaded.")
...
with st.expander("ℹ️ About This Model"):
    ...
    **Clinical Variables Used:**
    ...
    - **Symptoms**: Chest pain assessment
    - **Laboratory Tests**: 
      - Lipid panel (Total cholesterol, HDL, LDL, triglycerides)
      - Glucose metabolism (HbA1c, glucose, random glucose)
      - Kidney function (creatinine, BUN, electrolytes)
      - Liver function (AST)
      - Complete blood count (hemoglobin, hematocrit, WBC, RBC, platelets)
      - Additional markers (CPK, uric acid, blood cell indices)
    - **Medical History**: Family history of CHD, diabetes status, hypertension treatment, medications
    
    **Model Performance:**
    - The model has been trained on clinical data to distinguish between anginal and non-anginal chest pain
    - Uses advanced gradient boosting algorithms for high accuracy
    - Incorporates comprehensive clinical variables for robust prediction
    - Results should always be interpreted by qualified healthcare professionals
    
    **Risk Categories:**
    - **Low Risk (< 30%)**: Low probability of anginal chest pain
    - **Moderate Risk (30-70%)**: Moderate probability, consider further evaluation
    - **High Risk (> 70%)**: High probability, immediate medical attention recommended
    """)
...
st.markdown(f"""
<div class="footer">
    <p>🫀 CardioPredict AI • Powered by PyCaret & LightGBM</p>
    <p>⚠️ <strong>Medical Disclaimer:</strong> This tool is for educational and research purposes only and should not replace professional medical diagnosis or clinical judgment.</p>
    <p>🔬 <strong>Model:</strong> LightGBM via PyCaret | 📊 <strong>Framework:</strong> Streamlit</p>
